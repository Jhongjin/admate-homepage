    ...
    <div className="grid gap-3 lg:grid-cols-[minmax(260px,1.25fr)_repeat(5,minmax(130px,1fr))]">
      <Card className="min-w-0 overflow-hidden p-4 shadow-none">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-[11px] font-semibold uppercase text-[#9A9A9A]">전체 평균 진행률</div>
            <div className="mt-2 text-3xl font-semibold leading-none text-[#0D0D0D]">{overallProgress}%</div>
          </div>
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#ECEDF9] text-[#5E6AD2]">
      ...
      {items.map((item) => (
        <Card key={item.label} className="min-w-0 overflow-hidden p-4 shadow-none">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[11px] font-semibold text-[#9A9A9A]">{item.label}</div>
